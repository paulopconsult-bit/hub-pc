...
# Bucket de Observabilidade — Google Cloud Monitoring como solução nativa GCP.
# Datadog como evolução futura para ambientes multi-cloud ou alertas avançados.
resource "google_storage_bucket" "observability" {
  name                        = "hub-pc-dev-stg-observability"
  project                     = var.project_id
  location                    = var.regiao
  storage_class               = "STANDARD"
  uniform_bucket_level_access = true
