...
resource "google_artifact_registry_repository" "lakehouse" {
  ...
  format        = "DOCKER"
  description = "Repositorio de imagens Docker - microsservico 00-cliente (prod)"
...
# Pode ser disparado manualmente, pelo Scheduler ou pelo CI/CD.
# ==============================================================================
resource "google_cloud_run_v2_job" "pipeline_cliente" {
  project  = var.project_id
  name     = "job-lakehouse-00-cliente-prod"
  location = var.regiao
    ...
    template {
      ...
      containers {
        ...
        env {
          ...
          value = "hub-pc-prod-stg-lakehouse"
        }
        env {
          name  = "BUCKET_DW"
          value = "hub-pc-prod-stg-dw"
        }
      }
    }
  }
...
resource "google_cloud_scheduler_job" "scheduler_cliente" {
  ...
  http_target {
    http_method = "POST"
    uri         = "https://${var.regiao}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${var.project_id}/jobs/job-lakehouse-00-cliente-prod:run"
